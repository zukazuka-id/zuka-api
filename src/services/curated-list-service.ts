...
async function getListRestaurants(listId: string, lat?: number, lng?: number) {
  const distanceExpr =
    lat && lng
      ? sql`round(cast(ST_DistanceSphere(
            ST_MakePoint(${outlet.lng}, ${outlet.lat}),
            ST_MakePoint(${lng}, ${lat})
          ) / 1000 as numeric), 1) as distance`
  ...
  const rows = await db
    ...
    .orderBy(curatedListRestaurant.sortOrder)
    .limit(10);
...
export async function getAllCuratedLists(includeInactive = false) {
  return includeInactive
    ? db.select().from(curatedList).orderBy(curatedList.sortOrder)
    : db.select().from(curatedList).where(eq(curatedList.isActive, true)).orderBy(curatedList.sortOrder);
}
